*** Settings ***
Documentation     A test suite for validating Application Service Availability Query (APPSAQ) operations.
...
TC_MEC_SRV_APPSAQ_001_OK
    [Documentation]    Check that the IUT responds with a list of available MEC services
...
TC_MEC_SRV_APPSAQ_001_BR
    [Documentation]    Check that the IUT responds with an error when
...
TC_MEC_SRV_APPSAQ_002_OK
    ...
    ${SERVICE_ID}=    Get From Dictionary    ${response['body']}    serInstanceId
...
TC_MEC_SRV_APPSAQ_003_OK
    [Documentation]    Check that the IUT responds with the information on a specific service
    ...    for a given application instance when queried by a MEC Application
    ...
...
TC_MEC_SRV_APPSAQ_004_OK
    [Documentation]    Check that the IUT updates a service information for a given
